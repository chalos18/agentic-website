...
function buildShapes(def: PuzzleDefinition): PieceShape[] {
    ...
    for (const fragment of fragments) {
        ...
        const local = translatePolytope(fragment, [-centre[0], -centre[1], -centre[2]]);
...
export class TwistyPuzzle {
    ...
    turnSpec(move: string): TurnSpec | null {
        ...
        this.placements.forEach((placement, i) => {
            ...
            if (along > threshold + 1e-4 && along < (ceiling ?? Infinity) + 1e-4) pieceIds.push(i);
        });
        // Positive turns are clockwise seen from outside the face, which is a
        // negative rotation under the right-hand rule.
        return { move, axis, angle: (-2 * Math.PI * parsed.turns) / order, pieceIds };
    }
    ...
    applyMoves(moves: string[], record = true): TwistyPuzzle {
        return moves.reduce<TwistyPuzzle>((puzzle, move) => puzzle.applyMove(move, record), this);
    }
    ...
    scrambleSequence(count = this.def.scrambleLength): string[] {
        ...
        const suffixes = ['', "'", ...(this.def.moves[pool[0]].order === 4 ? ['2'] : [])];
        const moves: string[] = [];
        let previous = '';
    ...
    disorder(): number {
        let moved = 0;
        for (let i = 0; i < this.shapes.length; i++) {
            if (length(sub(this.placements[i].position, this.shapes[i].home)) > 1e-3) moved++;
        }
