...
export default function RequestBoard() {
  ...
  const {
    ...
    error,
    submit,
  } = useSubmittableList<RecipeRequest, { recipeName: string; note: string }>({
    endpoint: "/api/recipe-requests",
    listKey: "requests",
    itemKey: "request",
    insertAt: "start",
    toPayload: (fields) => ({ ...fields, submitterName: getAnonName() }),
  });

  const handleSubmit = async (event: FormEvent) => {
    event.preventDefault();
    if (!recipeName.trim()) return;
    if (await submit({ recipeName, note }, "Could not submit request")) {
      setRecipeName("");
      setNote("");
    }
  };
  ...
  return (
    <div>
      <form
        ...
        className="mb-8 p-5 rounded-2xl bg-cream-deep border border-clay-line space-y-3"
      >
        <h2 className="font-display text-lg font-semibold text-espresso">
          Request a recipe
        </h2>
        <p className="text-sm text-espresso-light">
          Tried something great, or want to see me post one of yours? Drop it
          here — everyone can see the list.
        </p>
        <input
          value={recipeName}
      ...
      <div className="space-y-3">
        {requests === null && (
          <p className="text-sm text-espresso-light/70">Loading requests…</p>
        )}
        {requests?.length === 0 && (
        ...
        {requests?.map((req) => (
          <div
            key={req.id}
            className="p-4 rounded-xl bg-cream border border-clay-line"
          >
            ...
            {req.note && (
              <p className="text-sm text-espresso-light mb-2">{req.note}</p>
            )}
