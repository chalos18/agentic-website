...
import { parseMoveSequence } from "@/services/puzzles/TwistyPuzzle";

interface AlgorithmInputProps {
  onMovesParsed?: (_moves: string[]) => void;
...
const EXAMPLES_BY_PUZZLE: Partial<
  Record<PuzzleId, { name: string; moves: string }[]>
> = {
  "2x2": [
...
export default function AlgorithmInput({
  ...
  const handleParse = () => {
    ...
    try {
      const moves = parseMoveSequence(input, PUZZLES[puzzleId]);
      setParsedMoves(moves);
      setError(null);
      onMovesParsed?.(moves);
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Invalid move notation";
      setError(message);
      onError?.(message);
    }
  };
    ...
    <div className="bg-cream-deep rounded-2xl p-6 border border-clay-line space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-espresso-light">
          Separate moves with spaces. Use single quotes for inverse (e.g.,
          R&apos;)
        </p>
      </div>
      ...
      <div className="bg-cream rounded-lg p-3 space-y-1 border border-clay-line">
        ...
        <ul className="text-xs text-espresso-light space-y-1 list-disc list-inside">
          <li>
            {Object.keys(PUZZLES[puzzleId].moves).join(", ")} for this
            puzzle&apos;s moves
          </li>
